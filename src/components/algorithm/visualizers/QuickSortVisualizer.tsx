...
import {
  ...
  generateReversed,
} from '@/utils/algorithm/quickSort'
...
const QUICKSORT_CODE = `// 퀵정렬 (Quick Sort)
function quickSort(arr, low, high) {
  if (low < high) {
    const pivotIdx = partition(arr, low, high);
    quickSort(arr, low, pivotIdx - 1);   // 왼쪽 부분
    quickSort(arr, pivotIdx + 1, high);   // 오른쪽 부분
  }
}
...
function partition(arr, low, high) {
  ...
  swap(arr, i + 1, high);    // 피벗 배치
  return i + 1;
}`

// Map step action → highlighted line numbers (1-indexed)
function getHighlightLines(action: QuickSortStep['action']): number[] {
  switch (action) {
    case 'pivot-select': return [11]  // const pivot = arr[high]
    case 'compare':      return [15]  // if (arr[j] <= pivot)
    case 'swap':         return [17]  // swap(arr, i, j)
    case 'no-swap':      return [15]
    case 'pivot-place':  return [20]  // swap(arr, i + 1, high)
    case 'done':         return [3]   // if (low < high) — base case
    default:             return []
  }
}

export default function QuickSortVisualizer() {
  const t = useTranslations('quickSortVisualizer')
  ...
  const [sortResult, setSortResult] = useState<QuickSortResult | null>(null)
  ...
  const currentStep = useMemo<QuickSortStep | null>(() => {
  ...
  const visualPivot = currentStep?.pivot ?? null
  ...
  const visualPartitionRange = currentStep?.partitionRange ?? null
        ...
        <div className="xl:col-span-3 space-y-4">
          <div className="backdrop-blur-xl bg-white/70 dark:bg-gray-800/70 border border-white/20 dark:border-gray-700/30 rounded-2xl p-4 space-y-4">
            ...
            <div className="flex justify-center">
              <QuickSortCanvas2D
                ...
                sorted={visualSorted}
                partitionRange={visualPartitionRange}
                width={600}
                height={360}
              />
            </div>
          ...
          <div className="backdrop-blur-sm bg-white/50 dark:bg-gray-800/50 border border-white/20 dark:border-gray-700/30 rounded-xl p-4 space-y-3">
            ...
            <div className="flex flex-wrap gap-3 text-xs text-gray-500 dark:text-gray-400">
              ...
              <span className="flex items-center gap-1">
                <span className="w-3 h-3 rounded-sm border border-dashed border-orange-400" />
                {t('grid.partition')}
              </span>
            </div>
          </div>
...
function QuickSortStepsList({
  steps,
  currentIndex,
  onStepClick,
  t,
}: {
  steps: QuickSortStep[] | undefined
  currentIndex: number
  ...
  const displaySteps = useMemo(() => {
    ...
    return steps
      ...
      .filter(s =>
        s.action === 'pivot-select' ||
        s.action === 'compare' ||
        s.action === 'swap' ||
        s.action === 'pivot-place' ||
        s.action === 'done'
      )
  }, [steps])
  ...
  return (
    <div ref={listRef} className="space-y-1">
      {displaySteps.map((step, i) => {
        ...
        // Render depth divider when pivot-select starts a new partition
        const showDepthDivider =
          step.action === 'pivot-select' && step.depth !== lastRenderedDepth
        if (step.action === 'pivot-select') {
        ...
        if (step.action === 'pivot-select') {
          icon = '📌'
          label = t('stepsGuide.pivotSelect', { idx: String(step.pivot ?? '') })
        ...
        } else if (step.action === 'pivot-place') {
          icon = '✅'
          label = t('stepsGuide.pivotPlace', { idx: String(step.pivot ?? '') })
        ...
        return (
          <div key={i}>
            {/* Depth divider on new pivot-select */}
            {showDepthDivider && (
              <div className="flex items-center gap-2 my-2">
                ...
                {step.partitionRange && (
                  <span className="text-[10px] px-2 py-0.5 rounded-full bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400 font-medium">
                    [{step.partitionRange[0]}–{step.partitionRange[1]}]
                  </span>
                )}
                <div className="flex-1 h-px bg-gray-200 dark:bg-gray-700" />
              </div>
