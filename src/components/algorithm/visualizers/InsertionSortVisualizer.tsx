...
import {
  ...
  generateReversed,
} from '@/utils/algorithm/insertionSort'
...
const INSERTION_SORT_CODE = `// 삽입정렬 (Insertion Sort)
function insertionSort(arr) {
  for (let i = 1; i < arr.length; i++) {
    const key = arr[i];           // 현재 요소 선택
    let j = i - 1;

    while (j >= 0 && arr[j] > key) {  // 비교
...
function getHighlightLines(action: InsertionSortStep['action']): number[] {
  switch (action) {
    case 'select':  return [4]   // const key = arr[i]
    case 'compare': return [7]   // while (j >= 0 && arr[j] > key)
    case 'shift':   return [8]   // arr[j + 1] = arr[j]
    case 'insert':  return [11]  // arr[j + 1] = key
    case 'done':    return [2]   // for loop header — algorithm complete
    default:        return []
  }
}

export default function InsertionSortVisualizer() {
  const t = useTranslations('insertionSortVisualizer')
  ...
  const [sortResult, setSortResult] = useState<InsertionSortResult | null>(null)
  ...
  const currentStep = useMemo<InsertionSortStep | null>(() => {
  ...
  const visualCurrent = currentStep?.current ?? null
  const visualComparing = currentStep?.comparing ?? null
  const visualShifting = currentStep?.shifting ?? null
  const visualInsertAt = currentStep?.insertAt ?? null
  ...
  const shifts = currentStep?.shifts ?? 0
      ...
      <div className="grid xl:grid-cols-5 gap-6">
        ...
        <div className="xl:col-span-3 space-y-4">
          <div className="backdrop-blur-xl bg-white/70 dark:bg-gray-800/70 border border-white/20 dark:border-gray-700/30 rounded-2xl p-4 space-y-4">
            ...
            {/* Canvas */}
            <div className="flex justify-center">
              <InsertionSortCanvas2D
                array={visualArray}
                current={visualCurrent}
                comparing={visualComparing}
                shifting={visualShifting}
                insertAt={visualInsertAt}
                sorted={visualSorted}
                width={600}
                height={360}
              />
...
function SortStepsList({
  ...
  steps: InsertionSortStep[] | undefined
  currentIndex: number
  ...
  return (
    <div ref={listRef} className="space-y-1">
      {displaySteps.map((step, i) => {
        ...
        // Render divider when outer loop index changes (new element being inserted)
        const showDivider =
          step.action === 'select' && step.current !== lastRenderedCurrent
        if (step.action === 'select' && step.current !== null) {
          lastRenderedCurrent = step.current
        }
        ...
        return (
          <div key={i}>
            {/* Divider for each new outer iteration */}
            {showDivider && (
              ...
              <div className="flex items-center gap-2">
                ...
                {isActive && step.action !== 'done' && (
                  <span className="ml-auto text-[10px] px-1.5 py-0.5 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 flex-shrink-0 tabular-nums">
                    {step.comparisons}c / {step.shifts}s
                  </span>
                )}
              </div>
            </div>
