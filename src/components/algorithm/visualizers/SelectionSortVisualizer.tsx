...
import {
  ...
  generateReversed,
} from '@/utils/algorithm/selectionSort'
...
const SELECTION_SORT_CODE = `// 선택정렬 (Selection Sort)
function selectionSort(arr) {
  ...
  for (let i = 0; i < n - 1; i++) {
    ...
    for (let j = i + 1; j < n; j++) {
      if (arr[j] < arr[minIdx]) { // 비교
        minIdx = j;               // 새 최솟값
      }
    }

    if (minIdx !== i) {
      swap(arr, i, minIdx);       // 스왑
    }
  }
}`

// Map step action → highlighted line numbers (1-indexed)
function getHighlightLines(action: SelectionSortStep['action']): number[] {
  switch (action) {
    case 'scan-start':  return [6]   // let minIdx = i
    case 'compare':     return [9]   // if (arr[j] < arr[minIdx])
    case 'new-min':     return [10]  // minIdx = j
    case 'swap':        return [15]  // swap(arr, i, minIdx)
    case 'no-swap':     return [14]  // if (minIdx !== i)
    case 'done':        return [2]   // function selectionSort
    default:            return []
  }
}

export default function SelectionSortVisualizer() {
  const t = useTranslations('selectionSortVisualizer')
  ...
  const [sortResult, setSortResult] = useState<SelectionSortResult | null>(null)
  ...
  const currentStep = useMemo<SelectionSortStep | null>(() => {
  ...
  const visualMinIndex = currentStep?.minIndex ?? null
  ...
  const visualScanRange = currentStep?.scanRange ?? null
      ...
      <div className="grid xl:grid-cols-5 gap-6">
        ...
        <div className="xl:col-span-3 space-y-4">
          <div className="backdrop-blur-xl bg-white/70 dark:bg-gray-800/70 border border-white/20 dark:border-gray-700/30 rounded-2xl p-4 space-y-4">
            ...
            {/* Canvas */}
            <div className="flex justify-center">
              <SelectionSortCanvas2D
                array={visualArray}
                current={visualCurrent}
                minIndex={visualMinIndex}
                comparing={visualComparing}
                swapping={visualSwapping}
                sorted={visualSorted}
                scanRange={visualScanRange}
                width={600}
                height={360}
              />
            </div>
          ...
          <div className="backdrop-blur-sm bg-white/50 dark:bg-gray-800/50 border border-white/20 dark:border-gray-700/30 rounded-xl p-4 space-y-3">
            ...
            <div className="flex flex-wrap gap-3 text-xs text-gray-500 dark:text-gray-400">
              ...
              <span className="flex items-center gap-1">
                <span className="w-3 h-3 rounded-sm bg-yellow-400 opacity-60" />
                {t('grid.comparing')}
              </span>
              <span className="flex items-center gap-1">
                <span className="w-3 h-3 rounded-sm bg-red-400 opacity-70" />
                {t('grid.swapping')}
              </span>
              <span className="flex items-center gap-1">
...
function SelectionStepsList({
  steps,
  currentIndex,
  onStepClick,
  t,
}: {
  steps: SelectionSortStep[] | undefined
  currentIndex: number
  ...
  // Filter to show meaningful steps
