...
import {
  type GraphNode, type GraphEdge, type KruskalStep, type KruskalResult,
  kruskal, generateRandomGraph,
} from '@/utils/algorithm/kruskal'
import KruskalCanvas2D from './KruskalCanvas2D'
...
const KRUSKAL_CODE = `// 크루스칼 최소 신장 트리 알고리즘
function kruskal(nodes, edges) {
  edges.sort((a, b) => a.weight - b.weight);
  const parent = makeSet(nodes.length);
...
export default function KruskalVisualizer() {
  const t = useTranslations('kruskalVisualizer')
  ...
  const currentStep: KruskalStep | null = useMemo(() => {
  ...
  // Compute cumulative skipped edges up to current step
  const skippedEdges = useMemo(() => {
    const set = new Set<number>()
    if (!result) return set
    for (let i = 0; i <= currentStepIndex && i < result.steps.length; i++) {
      if (result.steps[i].action === 'skip') set.add(result.steps[i].edgeIndex)
    }
    return set
  }, [result, currentStepIndex])
  ...
  const codeHighlightLines = useMemo(() => {
    ...
    if (currentStep.action === 'sort') return [2]
    if (currentStep.action === 'check') return [6, 7]
    if (currentStep.action === 'add') return [10, 11]
    if (currentStep.action === 'skip') return [13]
    if (currentStep.action === 'done') return [16]
    return []
  }, [currentStep])
        ...
        <div className="xl:col-span-3 space-y-4">
          <div className="backdrop-blur-xl bg-white/70 dark:bg-gray-800/70 border border-white/20 dark:border-gray-700/30 rounded-2xl p-4 space-y-4">
            ...
            <div className="flex justify-center">
              <KruskalCanvas2D
                nodes={nodes}
                edges={edges}
                sortedEdges={result?.edgesSorted ?? []}
                mstEdgeIndices={currentStep?.mstEdges ?? []}
                currentEdgeIndex={currentStep?.edgeIndex ?? -1}
                skippedEdges={skippedEdges}
                parent={currentStep?.parent ?? nodes.map((_, i) => i)}
                isRunning={isRunning}
          ...
          <div className="backdrop-blur-sm bg-white/50 dark:bg-gray-800/50 border border-white/20 dark:border-gray-700/30 rounded-xl p-4 space-y-3">
            ...
            <div className="flex flex-wrap gap-3 text-xs text-gray-500 dark:text-gray-400">
              ...
              <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-sm bg-red-500" /> {t('legend.skipped')}</span>
                ...
                {activeTab === 'steps' && (
                  <div className="space-y-2">
                    ...
                    {currentStepIndex < 0 ? (
                      ...
                      <KruskalStepsList steps={result?.steps} currentIndex={currentStepIndex} onStepClick={setCurrentStepIndex} t={t} />
                    )}
                  </div>
                )}
                {activeTab === 'code' && (
                  <CodeViewer code={KRUSKAL_CODE} language="javascript" highlightLines={codeHighlightLines} title="kruskal.js" />
                )}
                {activeTab === 'guide' && (
                  <GuideSection namespace="kruskalVisualizer" defaultOpen />
                )}
              </div>
            </div>
          </div>
...
function KruskalStepsList({ steps, currentIndex, onStepClick, t }: {
  steps: KruskalStep[] | undefined
  currentIndex: number
  ...
  return (
    <div ref={listRef} className="space-y-1">
      {steps.map((step, i) => {
        ...
        let label = ''
        if (step.action === 'sort') label = t('stepsGuide.sorted')
        else if (step.action === 'check' && step.edge) label = t('stepsGuide.checking', { from: String.fromCharCode(65 + step.edge.from), to: String.fromCharCode(65 + step.edge.to), w: String(step.edge.weight) })
        else if (step.action === 'add' && step.edge) label = t('stepsGuide.added', { from: String.fromCharCode(65 + step.edge.from), to: String.fromCharCode(65 + step.edge.to) })
        else if (step.action === 'skip' && step.edge) label = t('stepsGuide.skipped', { from: String.fromCharCode(65 + step.edge.from), to: String.fromCharCode(65 + step.edge.to) })
        else if (step.action === 'done') label = t('stepsGuide.done', { w: String(step.totalWeight) })

        const icon = step.action === 'sort' ? '📋' : step.action === 'add' ? '✅' : step.action === 'skip' ? '❌' : step.action === 'done' ? '🏁' : '🔍'
