...
export default function DotsAndBoxes({ initialRoom, isHost: isHostProp, hostPeerId, onBack }: DotsAndBoxesProps) {
  ...
  useEffect(() => {
    ...
    const joinInitialRoom = async () => {
      ...
      const dotsNickname = localStorage.getItem('dotsandboxes_player_name')
      const name = gameNickname || dotsNickname || t('guest')
      setPlayerName(name)
      if (!dotsNickname) localStorage.setItem('dotsandboxes_player_name', name)
