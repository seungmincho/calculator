...
export default function Battleship({ initialRoom, isHost: isHostProp, hostPeerId, onBack }: BattleshipProps) {
  ...
  useEffect(() => {
    ...
    const joinInitialRoom = async () => {
      ...
      const battleshipNickname = localStorage.getItem('battleship_player_name')
      const name = gameNickname || battleshipNickname || t('guest')
      setPlayerName(name)
      if (!battleshipNickname) localStorage.setItem('battleship_player_name', name)
