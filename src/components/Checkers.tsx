...
export default function Checkers({ initialRoom, isHost: isHostProp, hostPeerId, onBack }: CheckersProps) {
  ...
  useEffect(() => {
    ...
    if (setupInProgressRef.current) {
      console.log('[Checkers] Setup already in progress, skipping...')
      return
    }

    console.log('[Checkers] Processing initialRoom:', initialRoom.id, 'isHost:', isHostProp)
    initialRoomIdRef.current = initialRoom.id
    setupInProgressRef.current = true

    // 방 생성한 호스트인 경우 - GameHub에서 이미 PeerJS 방 생성됨
    if (isHostProp) {
      console.log('[Checkers] Setting up as host (PeerJS already created by GameHub)')
    ...
    console.log('[Checkers] Setting up as guest')
    const joinInitialRoom = async () => {
      // GameHub에서 넘어온 경우 gameNickname 사용, 아니면 checkers_player_name 사용
      const gameNickname = localStorage.getItem('gameNickname')
      const checkersNickname = localStorage.getItem('checkers_player_name')
      const name = gameNickname || checkersNickname || t('guest')
      setPlayerName(name)
      if (!checkersNickname) localStorage.setItem('checkers_player_name', name)
      ...
      // PeerJS로 호스트에 연결
      console.log('[Checkers] Connecting to host:', initialRoom.host_id)
      const success = await joinPeerRoomRef.current(initialRoom.host_id)
      console.log('[Checkers] Join result:', success)
      setupInProgressRef.current = false
      if (!success) {
