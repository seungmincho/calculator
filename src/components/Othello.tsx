...
export default function Othello({ initialRoom, isHost: isHostProp, hostPeerId, onBack }: OthelloProps) {
  ...
  useEffect(() => {
    ...
    if (setupInProgressRef.current) {
      console.log('[Othello] Setup already in progress, skipping...')
      return
    }

    console.log('[Othello] Processing initialRoom:', initialRoom.id, 'isHost:', isHostProp)
    initialRoomIdRef.current = initialRoom.id
    setupInProgressRef.current = true

    // 방 생성한 호스트인 경우 - GameHub에서 이미 PeerJS 방 생성됨
    if (isHostProp) {
      console.log('[Othello] Setting up as host (PeerJS already created by GameHub)')
    ...
    console.log('[Othello] Setting up as guest')
    const joinInitialRoom = async () => {
      // GameHub에서 넘어온 경우 gameNickname 사용, 아니면 othello_player_name 사용
      const gameNickname = localStorage.getItem('gameNickname')
      const othelloNickname = localStorage.getItem('othello_player_name')
      const name = gameNickname || othelloNickname || t('guest')
      setPlayerName(name)
      if (!othelloNickname) localStorage.setItem('othello_player_name', name)
      ...
      // PeerJS로 호스트에 연결
      console.log('[Othello] Connecting to host:', initialRoom.host_id)
      const success = await joinPeerRoomRef.current(initialRoom.host_id)
      console.log('[Othello] Join result:', success)
      setupInProgressRef.current = false
      if (!success) {
