    ...
    try {
      ...
      if (activeTab === 'jsonToCsv') {
        ...
        if (!Array.isArray(jsonData)) {
          throw new Error(t('errors.notArray'))
        }
                    ...
                    {tableData.length > rowsPerPage && (
                      <div className="px-4 py-3 bg-gray-50 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600 flex justify-between items-center">
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <button
                            onClick={() => setCurrentPage(Math.min(Math.ceil(tableData.length / rowsPerPage), currentPage + 1))}
                      ...
                      <table className="w-full text-sm">
                        <thead className="bg-gray-50 dark:bg-gray-700 sticky top-0">
                          <tr>
                            {tableHeaders.map((header, index) => (
                              <th key={index} className="px-4 py-2 text-left font-medium text-gray-900 dark:text-white border-r border-gray-200 dark:border-gray-600 last:border-r-0 min-w-[100px]">
                                {header}
                              </th>
                            ))}
                          </tr>
                        ...
                        <tbody className="divide-y divide-gray-200 dark:divide-gray-600">
                          {tableData
                            ...
                            .map((row, rowIndex) => (
                            <tr key={rowIndex} className={`${rowIndex % 2 === 0 ? 'bg-white dark:bg-gray-800' : 'bg-gray-50 dark:bg-gray-700'} hover:bg-blue-50 dark:hover:bg-blue-900/20`}>
                              {tableHeaders.map((header, colIndex) => (
                                <td key={colIndex} className="px-4 py-2 text-gray-900 dark:text-white border-r border-gray-200 dark:border-gray-600 last:border-r-0">
                                  {row[header] || ''}
                                </td>
                              ))}
                            </tr>
