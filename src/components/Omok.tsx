...
export default function Omok({ initialRoom, isHost: isHostProp, onBack, joinPeerId }: OmokProps) {
  ...
  useEffect(() => {
    ...
    const autoJoin = async () => {
      const gameNickname = localStorage.getItem('gameNickname')
      const savedName = localStorage.getItem('omok_player_name')
      const name = gameNickname || savedName || t('guest')
      setPlayerName(name)
      if (!savedName) localStorage.setItem('omok_player_name', name)
      isHostRef.current = false
      setChatMessages([])
      setGamePhase('waiting')
    ...
    return (
      <div className="max-w-2xl mx-auto text-center">
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-8">
          ...
          <GameInviteLink peerId={peerId} gameSlug="omok" gameTitle={t('title')} />
