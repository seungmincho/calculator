...
Codex 세션 히스토리를 분석하고 인사이트를 추출합니다.
...
```
~/.Codex/projects/<encoded-cwd>/*.jsonl
```
...
2. **명시되지 않은 경우** - AskUserQuestion 호출:
   ...
   options:
     - "현재 프로젝트만" → ~/.Codex/projects/<encoded-cwd>/*.jsonl
     - "모든 Codex 세션" → ~/.Codex/projects/**/*.jsonl
   ```
...
find ~/.Codex/projects/<encoded-cwd> -name "*.jsonl" -type f

# All sessions (모든 프로젝트)
find ~/.Codex/projects -name "*.jsonl" -type f
```
