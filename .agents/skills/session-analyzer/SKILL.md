...
description: This skill should be used when the user asks to "analyze session", "세션 분석", "evaluate skill execution", "스킬 실행 검증", "check session logs", "로그 분석", provides a session ID with a skill path, or wants to verify that a skill executed correctly in a past session. Post-hoc analysis of Codex sessions to validate skill/agent/hook behavior against SKILL.md specifications.
version: 1.0.0
user-invocable: true
---
...
Post-hoc analysis tool for validating Codex session behavior against SKILL.md specifications.
...
Session files are located in `~/.Codex/`:

```bash
# Main session log
~/.Codex/projects/-{encoded-cwd}/{sessionId}.jsonl

# Debug log (detailed)
~/.Codex/debug/{sessionId}.txt

# Agent transcripts (if subagents were used)
~/.Codex/projects/-{encoded-cwd}/agent-{agentId}.jsonl
```
...
The debug log (`~/.Codex/debug/{sessionId}.txt`) contains detailed execution traces.
...
```
User: "Analyze session 3cc71c9f-d27a-4233-9dbc-c4f07ea6ec5b against .Codex/skills/specify/SKILL.md"
